...
- Лайк / Пропустить.
- Взаимный лайк => матч.
- Без чата: сразу сценарий встречи.
- 24 часа на подтверждение встречи (иначе матч удаляется).
...
- `/start` — регистрация (FSM)
- `/skip_photo` — пропустить фото на этапе регистрации
- `/browse` — показать случайную анкету в своём городе
